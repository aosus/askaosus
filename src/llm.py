...
class LLMClient:
    ...
    def _get_default_system_prompt(self) -> str:
        ...
        return """أنت مساعد ذكي لمنتدى آسوس، المجتمع العربي الأكبر للبرمجيات الحرة والمفتوحة المصدر.

مهمتك هي مساعدة المستخدمين بالإجابة على أسئلتهم بناءً على محتوى المنتدى. 

قواعد الإجابة:
1. اكتب إجابة قصيرة ومفيدة باللغة العربية
2. استند على المحتوى المقدم من المنتدى
3. اذكر رابط المنشور الأكثر صلة للمزيد من التفاصيل
4. شجع المستخدم على زيارة المنتدى للاستفادة أكثر
5. إذا لم تجد إجابة مناسبة، اعتذر ووجه المستخدم لطرح السؤال في المنتدى

احرص على أن تكون الإجابة مختصرة (أقل من 500 كلمة) ومشجعة للمستخدم لزيارة المنتدى."""
...
"""
            ...
            answer += f"\n\n💬 للمزيد من المناقشات والمساعدة، تفضل بزيارة منتدى آسوس: {self.config.discourse_base_url}"
            
            return answer
            
        except Exception as e:
            logger.error(f"Error generating answer: {e}", exc_info=True)
            return "عذراً، حدث خطأ أثناء توليد الإجابة. يرجى المحاولة مرة أخرى أو زيارة منتدى آسوس مباشرة: https://discourse.aosus.org"
